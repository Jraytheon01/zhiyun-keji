...
收到上述极简任务后，直接按本 Skill 完成课程读取、证据检索、对话组织、产物生成和结束回流；不要要求平台再次提供工具顺序或回流格式。
只使用 `zhiyun-learning` MCP。即使 TeleAgent 同时配置了 `meeting-assistant`，也不得用它读取、搜索或回流本次课程。
...
- 复盘与检测中，只有学习者明确表示结束并保存时才回流；例如“结束复盘并回流课迹”“结束检测并回流课迹”或语义等价表达。一键生成产物的场景完成后即可回流。
